...
public class EarthquakeCityMap extends PApplet {
	...
	private void checkEarthquakesForClick() {
		...
		for (Marker quakeMarker : quakeMarkers) {
			EarthquakeMarker earthquarkMarker  = (EarthquakeMarker) quakeMarker;
			
			if (!earthquarkMarker.isHidden() && earthquarkMarker.isInside(map, mouseX, mouseY)) {
				lastClicked = earthquarkMarker;
				...
				for(Marker mhide : cityMarkers) {
					if (mhide.getDistanceTo(earthquarkMarker.getLocation()) > earthquarkMarker.threatCircle()) {
						mhide.setHidden(true);
					}
				}
				return;
	...
	private void checkCitiesForClick() {
		...
		for(Marker marker : cityMarkers) {
			if(!marker.isHidden() && marker.isInside(map, mouseX, mouseY)) {
				...
				for(Marker mhide : quakeMarkers) {
					EarthquakeMarker earthquakeMarker = (EarthquakeMarker) mhide;
