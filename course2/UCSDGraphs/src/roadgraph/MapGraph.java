...
public class MapGraph {
	...
	public double getKruskalMST() {
		List<MapEdge> mst = new ArrayList<MapEdge>();
		List<MapEdge> mstReturnPath = new ArrayList<MapEdge>();
		DisjointSet disJointSet = new DisjointSet();
		double costOfSpanningTree = 0.0;
		...
		for (MapEdge edge : edges) {
			GeographicPoint root1 = disJointSet.findSet(edge.getStart().getLocation());
			GeographicPoint root2 = disJointSet.findSet(edge.getEnd().getLocation());

			// check if the route in opposite direction already exist, if so add
			// it to the set
			for (MapEdge edgeExisting : mst) {
				if ((edgeExisting.getEnd().getLocation().equals(edge.getStart().getLocation()))
						&& (edgeExisting.getStart().getLocation().equals(edge.getEnd().getLocation()))) {
					mstReturnPath.add(edge);
				}
			}
			...
			if (root1 == root2) {
				...
				costOfSpanningTree += edge.getDistance();
			}
		}

		mst.addAll(mstReturnPath);

		System.out.println("Printing MST Edges....");

		for (MapEdge edge : mst) {
			System.out.println("Start: " + edge.getStart().getLocation() + ", End " + edge.getEnd().getLocation()
					+ ", Road ID: " + ", Road Name: " + edge.getRoadName() + ", Road Type: " + edge.getRoadType()
					+ ", Distance: " + edge.getDistance());
		}
		System.out.println("End of MST Edges....");

		return costOfSpanningTree;
	}
	...
	public static void main(String[] args) {
		...
		double costOfSpanningTree = theMap.getKruskalMST();
		System.out.println("Cost of Spanning Tree: " + costOfSpanningTree);
