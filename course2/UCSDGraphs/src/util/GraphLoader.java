...
public class GraphLoader {
	...
	/**
	 * * The file contains data lines as follows: lat1 lon1 lat2 lon2 roadName
	 * roadType
	 * 
	 * where each line is a segment of a road These road segments are assumed to
	 * be ONE WAY.
	 * 
	 * This method will collapse the points so that only intersections are
	 * represented as nodes in the graph.
	 * 
	 * @param roadDataFile
	 *            The file containing the road data, in the format described.
	 * @param intersectionsFile
	 *            The output file containing the intersections.
	 */
	 ...
	 *            The file containing the road data, in the format described.
	 * @param map
	 *            The graph to load the map into. The graph is assumed to be
	 *            directed.
	 * @param segments
	 *            The collection of RoadSegments that define the shape of a
	 *            road. These segments are maintained separately from the graph
	 *            as they are only used to display paths.
	 */
...
//		map.callSetNeighbors();
	...
	/**
	 * Read in a file specifying route maps between airports. The file contains
	 * data as follows: Airline, AirlineID, Source airport, Source airport ID,
	 * Destination airport, Destination airport ID, Codeshare, Stops, Equipment
	 * This method will only read in nonstop routes (with Stops == 0) Vertices
	 * are airports (labeled with Strings) Edges represent nonstop routes
	 * 
	 * @param filename
	 * @param graph
	 */
	public static void loadRoutes(String filename, Graph graph) {
		...
		try {
			...
			while ((nextLine = reader.readLine()) != null) {
				...
				// Add edge for this flight, if both source & destination are
				// already vertices.
				// If one of these airports is missing, add vertex for it and
				// then place edge.
	...
	/**
	 * Loads a graph from a file. The file is specified with each line
	 * representing an edge. Vertices are numbered from 0..1-numVertices.
	 * 
	 * The first line of the file contains a single int which is the number of
	 * vertices in the graph. e.g. 5 1 3 3 2 3 5 5 4
	 * 
	 * @param filename
	...
	public static void loadGraph(String filename, basicgraph.Graph theGraph) {
		...
		try {
			...
			// Read the lines out of the file and put them in a HashMap by
			// points
			while ((nextLine = reader.readLine()) != null) {
	...
	public static void main(String[] args) {
		...
		// intersections files
		// just change YOURFILE in the strings below to be the name of the file
		// you saved.
...
class RoadLineInfo {
	...
	/**
	 * Create a new RoadLineInfo object to store information about the line read
	 * from the file
	 * 
	 * @param p1
	 *            One of the points
	...
	/**
	 * Two RoadLineInfo objects are considered equal if they have the same two
	 * points and the same roadName and roadType.
	 */
	public boolean equals(Object o) {
		...
		return info.point1.equals(this.point1) && info.point2.equals(this.point2) && info.roadType.equals(this.roadType)
				&& info.roadName.equals(this.roadName);
	...
	/**
	 * Returns whether these segments are part of the same road in terms of road
	 * name and road type.
	 * 
	 * @param info
	 *            The RoadLineInfo to compare against.
